...
int insere_ord(Lista lst, int elem)
{
    ...
    if(lista_vazia(lst) == 1 || lst -> no[lst->fim-1])
    {
    ...
    else
    {
        ...
        while(elem >= lst->no[aux]) //pecorre a lista
        { 
            aux++;
        }
...
int remove_ord(Lista lst, int elem)
{
    ...
    while (aux < lst->fim && lst->no[aux] < elem)//percorre ate achar o elemento ou no maior, ou final da lista
    {
        aux++;
    }
    ...
    for (i = aux + 1; i < lst->fim; i++)//deslocamento a esquerda do sucessor ate o final da lsita
    {
        lst->no[i - 1] = lst->no[i];
    }
